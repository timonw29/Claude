...
from . import activity, config, gcalendar, goals, google_auth, hermes_client, memory, portfolio, profile, system_stats, todos, tts, weather, widgets
...
_state = {"messages": memory.load_history()}
...
def _run_turn(message):
    """Sends `message` (plus the full history) to Hermes and returns a
    dashboard-shaped result dict. Hermes has already executed any tool/skill
    calls server-side by the time this returns - see hermes_client.py for
    what that means for confirmation and activity logging."""
    _state["messages"].append({"role": "user", "content": message})
    try:
        text, latency_ms, prompt_tokens = hermes_client.chat(_state["messages"])
    except hermes_client.HermesError as e:
        _state["messages"].pop()  # don't persist a turn that never got a reply
        return {"status": "error", "text": str(e)}

    _last_call["latency_ms"] = latency_ms
    if prompt_tokens is not None:
        _last_call["input_tokens"] = prompt_tokens
    _state["messages"].append({"role": "assistant", "content": text})
    memory.save_history(_state["messages"])
    activity.log("Nachricht beantwortet")
...
def _generate_briefing():
    prompt = (
        f"{BRIEFING_MARKER} Erstelle mein tägliches Morgen-Briefing. Nutze deine "
        "Websuche-Fähigkeit für aktuelle Zahlen. Fasse kompakt zusammen (max. "
        "ca. 150 Wörter):\n"
        "1. Wichtigste Aktienindizes (DAX, S&P 500, Nasdaq) - aktueller Stand "
        "und Veränderung zum Vortag.\n"
        "2. Meine über die moni-assistant-Skill (scripts/portfolio.py list) "
        "abrufbaren Positionen - aktuelle Kurse und Tagesveränderung.\n"
        "Kein Smalltalk, keine Rückfragen - direkt die Fakten."
    )
    with _lock:
        _run_turn(prompt)  # _run_turn already rolls back the history on error
